  ...
  methods: {
    closePetition: function () {
      ...
      modal
        ...
        .cssanimate("fadeOutDown", { hide: false }, function () {
  ...
  methods: {
    randomize: function () {
      for (var i = 0; i < this.images.length; i++) {
        ...
        if (img.sort_rank === 0)
          img.sort_rank = Math.random() * this.images.length;
      }
...
function setContentAvailableSpace(modal, additional_margin = 0) {
  ...
  modal
    ...
    .css({ height: "calc(95vh - " + usedSpace + "px)" });
...
function openPetition(petitionID, auto_open = false, counter = 0) {
  ...
  if (!toPetition) {
    ...
    openPetitionLoop = setTimeout(function () {
      if (counter < 10) {
        // Only loop for a max of 2 seconds.
    ...
    modal
      .find(".modal-container")
      .cssanimate("fadeInUp", { hide: true }, function () {
    ...
    $(document).on("keyup", function (e) {
      if (e.keyCode === 27) {
        // Close the petition when you press the escape key.
...
function animateUpdate(petitionID) {
  ...
  var petition = $("#petitions-container").find(
    ...
    try {
      ...
      if (data["command"]) {
        ...
        if (command === "update-sigs") {
          ...
          if (websocket_debug)
            console.log(
              "Petition " + petition_id + " now has " + sigs + " signatures!"
            );
          ...
          if (modal_petition.id && modal_petition.id === petition_id) {
            ...
            if (websocket_debug)
              console.log(
                "Petition " +
                  petition_id +
                  " is open, Updating Modal signatures.."
              );
          ...
          if (websocket_debug)
            console.log("Update for petition " + petition_id);
          ...
          if (websocket_debug)
            console.log("Petition was marked as in progress, updating it.");
          petition_id = data["petition"]["petition_id"];
          if (petitions.map.hasOwnProperty(petition_id)) {
          ...
          if (websocket_debug) {
            console.log(
              "Trying to add the next page of petitions to the page."
            );
          ...
          if (websocket_debug)
            console.log("List size after: " + petitions.list.length);
        } else if (command === "all") {
          if (websocket_debug) {
            console.log(
              "Trying to add the next page of petitions to the page."
          ...
          petitions.list = [];
          let list = data["petitions"].filter(
            (item) =>
...
function reportPetitionModal(petition_id) {
  window.popup = new Modal({
    ...
    bodyButtons: [
      [
        "Report",
        "material material-button material-shadow margin-top margin-bottom transition",
        "reportPetition(" +
          petition_id +
          ', $("#report-petition-select").val())',
      ],
      [
        "Cancel",
        "material material-button material-shadow margin-top margin-bottom transition minimal",
        "window.popup.close()",
      ],
    ],
  });
...
function reportErrorModal() {
  window.errorModalInstance = new Modal({
    ...
    bodyButtons: [
      [
        "Okay",
        "material material-button minimal material-shadow margin-top margin-bottom transition",
        "window.errorModalInstance.close()",
      ],
    ],
  });
...
function reportPetition(petition_id, reason) {
  var allowedReasons = [
    ...
    "Hateful or abusive",
    "Against policy",
    "Harassing or violent",
  ];
  if (allowedReasons.includes(reason)) {
    $.post(
      "petition/report/" + petition_id,
      { reason: reason, csrfmiddlewaretoken: get_csrf() },
      (r) => {
        window.popup.close();
        if (!JSON.parse(r)) {
          reportErrorModal();
...
$(document).ready(function () {
  ...
  $(document).on("click", ".tag", function () {
    ...
    $("html, body").animate(
      { scrollTop: $("#petitions").offset().top - 100 },
      "slow"
    );
  });
  ...
  $(document).on("click", "#publish-button", function () {
    ...
    window.popup = new Modal({
      ...
      bodyContent:
        "<p class='padding-bottom'>You cannot edit or delete it once it is published.</p>",
      iconContainerClass: "text-highlight",
      iconClass: "md-48",
      iconText: "warning",
      bodyButtons: [
        ...
        [
          "Confirm",
          "material material-button material-shadow margin-bottom margin-top transition cursor",
  ...
  $(window).scroll(function () {
    ...
    if (win.scrollTop() + win.height() > $(document).height() - 100) {
      ...
      if (
        !petitions.searched &&
        !window.petitions.loading &&
        (!window.last_paginate ||
          (window.last_paginate && window.last_paginate.length !== 0))
      ) {
